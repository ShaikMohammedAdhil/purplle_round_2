...
1. The video pipeline or an external CCTV analytics system emits structured events.
...
The included pipeline can generate these events from either one video or a directory of camera videos. In multi-camera mode, each discovered video is processed independently and all generated events include the inferred `camera_id`.
