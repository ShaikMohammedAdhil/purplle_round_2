...
pytest tests/test_multi_camera_pipeline.py
```
...
### tests/test_multi_camera_pipeline.py

Validates Phase 13 multi-camera behavior:

- Directory discovery for `.mp4`, `.avi`, and `.mov` videos
- Camera ID assignment from filenames and fallback ordering
- Per-camera output directories
- Store-level aggregation summary
- Failure isolation when one camera fails
- Upload payloads preserve per-camera `camera_id`
...
- Multi-camera directory orchestration
- Per-camera event upload correctness
