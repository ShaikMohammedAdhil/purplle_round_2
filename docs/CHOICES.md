...
Multi-camera support is implemented in the CLI runner rather than by changing detection, tracking, or event generation internals. Each video is treated as an independent camera run with its own output directory and inferred `camera_id`, then the runner aggregates per-camera summaries. This preserves backward compatibility for the existing `--video` command.
